...
For an always-on installation, declare the installed executable, `stream` arguments, and `LAKE_DATA` through [Stado's service lifecycle](https://stado.wisent.com/docs/walkthrough-service-repair); the product catalog declares the unit as `com.wisent.compute.service.transcript-lake`, the host's one Transcript Lake process. Started by launchd as that unit, `stream` boots out the hand-made `com.wisent.transcript-lake-stream`, which ran a second identical streamer, and removes its launch agent; run by hand it retires nothing. Stado's declared supervisor owns subsequent repairs; there is no repository installation script. Vendor transcripts remain read-only, and `clean` still previews removal of rebuildable artifacts only.
