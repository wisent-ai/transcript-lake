...
```sh
export LAKE_DATA="$(mktemp -d)/must-remain-absent"
transcript-lake ingest --source unsupported-runtime
```
...
```sh
PATH="/usr/bin:/bin" transcript-lake query "SELECT * FROM sessions"
```
