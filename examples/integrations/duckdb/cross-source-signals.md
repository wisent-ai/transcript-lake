...
7. **Artifacts and side effects:** The CLI loads packaged, version-matched SQL, reads Lake, and attaches Oko read-only. DuckDB may install/cache its SQLite extension outside the Lake. No Oko rows are modified.
8. **Steps:**
...
9. **Verification:** Freshness compares the Oko index with each Lake runtime; frustration ranks matching sessions; overlap reports hook-blocked, frustrated, and shared session counts. The CLI selects packaged SQL from the installed version.
