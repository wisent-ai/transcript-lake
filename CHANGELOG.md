...
- Claude and Kimi subagent transcripts are ingested for the same reason: `~/.claude/projects/<encoded-cwd>/<id>/subagents/agent-*.jsonl` (2396 files, 1.09 GiB on this machine) and `~/.kimi-code/sessions/wd_*/session_*/agents/<agent>/wire.jsonl` beyond `agents/main` (292 files, 156 MiB). Claude sidechains repeat their parent `sessionId`, so they join that session with `extra.sidechain` set; a Kimi subagent wire carries no identifier at all and becomes session `<session_*>.<agent>`. Measured with `sources --json` before and after: claude 1608 → 4004 discovered files, kimi 3432 → 3724, omp 257 → 4306.
